...
describe('BasketItemBuilderTest', () => {
  ...
  it('should initialise with default random id', () => {
    const basketItem = new BasketItemBuilder().withLabel('banana').build();
  ...
  it('should create correct basket item', () => {
    const modifier = BasketItemModifier.from('bla', 'Cheeeese!', 100, 50.0, 'addon');
    const basketItem = new BasketItemBuilder().withId('123').withLabel('banana').withQuantity(2)
      .withMeasurement(1.25, 'kg')
      .withCategory('fruit')
  ...
  it('should allow creation from existing item', () => {
    ...
    const newItem = BasketItemBuilder.from(basketItem).withQuantity(4).build();
  ...
  it('should allow increment item quantity', () => {
    const basketItem = new BasketItemBuilder().withId('123').withLabel('banana').withQuantity(2)
      ...
      .withAmount(200)
      .build();
    const newItem = BasketItemBuilder.from(basketItem).incrementQuantity().build();
  ...
  it('should allow decrement item quantity', () => {
    const basketItem = new BasketItemBuilder().withId('123').withLabel('banana').withQuantity(2)
      ...
      .withAmount(200)
      .build();
    const newItem = BasketItemBuilder.from(basketItem).decrementQuantity().build();
  ...
  it('should allow offset positive item quantity', () => {
    const basketItem = new BasketItemBuilder().withId('123').withLabel('banana').withQuantity(2)
      ...
      .withAmount(200)
      .build();
    const newItem = BasketItemBuilder.from(basketItem).offsetQuantityBy(2).build();
    expect(newItem.quantity).toBe(4);
  });
  ...
  it('should allow offset negative item quantity', () => {
    const basketItem = new BasketItemBuilder().withId('123').withLabel('banana').withQuantity(2)
      ...
      .withAmount(200)
      .build();
    const newItem = BasketItemBuilder.from(basketItem).offsetQuantityBy(-1).build();
    expect(newItem.quantity).toBe(1);
  });

  it('with fractional quantity should use measuree if unit is set', () => {
    const basketItem = new BasketItemBuilder().withId('123').withLabel('banana').withAmount(200)
      .withFractionalQuantity(2.0, 'kg')
      .build();
  ...
  it('with fractional quantity should throw is no unit with fraction', () => {
    expect(() => {
      new BasketItemBuilder().withId('123').withLabel('banana').withAmount(200)
        .withFractionalQuantity(2.1)
        .build();
  ...
  it('should throw if trying to set amount after setting modifiers', () => {
    expect(() => {
      new BasketItemBuilder().withId('123').withLabel('banana').withQuantity(2)
        .withCategory('fruit')
  ...
  it('should caluclate amount if modifiers are set', () => {
    const modifier1 = BasketItemModifier.from('bla', 'Cheeeese!', 100, 50.0, 'addon');
    const modifier2 = BasketItemModifier.from('bla2', 'Cheeeese!2', undefined, -5.5, 'discount');
    const basketItem = new BasketItemBuilder().withId('123').withLabel('banana').withQuantity(2)
      .withMeasurement(1.25, 'kg')
      .withCategory('fruit')
