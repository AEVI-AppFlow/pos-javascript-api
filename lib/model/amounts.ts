...
export class Amounts extends Jsonable {
  ...
  public static fromAmounts(from: Amounts): Amounts {
    const obj = Amounts.from(from.baseAmount, from.currency, from.additionalAmounts);
  ...
  public getTotalAmountValue(): number {
    ...
    for (const key of Object.keys(this.additionalAmounts)) {
  ...
  public static addAmounts(a1: Amounts, a2: Amounts): Amounts {
    ...
    const newBaseAmount = a1.baseAmount + a2.baseAmount;
    const newAdditionals = a1.additionalAmounts;
    const a2Additionals = a2.additionalAmounts;
    for (const a2Key of Object.keys(a2Additionals)) {
      if (Object.prototype.hasOwnProperty.call(newAdditionals, a2Key)) {
  ...
  public static subtractAmounts(a1: Amounts, a2: Amounts, keepZeroAmountAdditionals: boolean = true): Amounts {
    ...
    const remainingBase = Math.max(a1.baseAmount - a2.baseAmount, 0);
    const newAdditionals = a1.additionalAmounts;
    const a2Additionals = a2.additionalAmounts;
    for (const a2Key of Object.keys(a2Additionals)) {
      if (Object.prototype.hasOwnProperty.call(newAdditionals, a2Key)) {
        const additionalRemainder = Math.max(newAdditionals[a2Key] - a2Additionals[a2Key], 0);
