...
export class AppMessage extends Jsonable {
  ...
  public static wrapPayment(payment: Payment, responseMechanism: string): AppMessage {
    const paymentData = new AdditionalData();
    paymentData.addData(AppMessageTypes.PAYMENT_MESSAGE, payment);
    const request = Request.from(payment.flowType, paymentData);
    request.deviceId = payment.deviceId;
    const appMessage = AppMessage.from(AppMessageTypes.PAYMENT_MESSAGE, request.toJson(), this.getInternalData(), responseMechanism);
    return appMessage;
  }
